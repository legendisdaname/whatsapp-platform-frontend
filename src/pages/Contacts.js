...
function Contacts() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [contactsRes, groupsRes] = await Promise.all([
        fetch(`${process.env.REACT_APP_API_URL || 'https://whatapi.streamfinitytv.com'}/api/contacts?session_id=${selectedSession}`),
        fetch(`${process.env.REACT_APP_API_URL || 'https://whatapi.streamfinitytv.com'}/api/contacts/groups?session_id=${selectedSession}`)
      ]);
  ...
  const createContact = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://whatapi.streamfinitytv.com'}/api/contacts`, {
        method: 'POST',
  ...
  const deleteContact = async (id) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://whatapi.streamfinitytv.com'}/api/contacts/${id}`, { method: 'DELETE' });
      const data = await response.json();
      if (data.success) {
        fetchData();
  ...
  const createGroup = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://whatapi.streamfinitytv.com'}/api/contacts/groups`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session_id: selectedSession,
  ...
  const deleteGroup = async (id) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://whatapi.streamfinitytv.com'}/api/contacts/groups/${id}`, { method: 'DELETE' });
      const data = await response.json();
      if (data.success) {
        fetchData();
        alert('✅ Group deleted');
  ...
  const importContacts = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'https://whatapi.streamfinitytv.com'}/api/import/contacts`, {
        method: 'POST',
        body: formData
      });
